...
function saveSettings () {
	...
	if (bits!='' && bits!=origbits) {
		...
		if (bits.Favicon&&isToF(bits.Favicon)) { setstor.Favicon=bits.Favicon; cool=true; }
...
function handleMouse (what) {
	...
	if (what.id=='divHeader') {
	...
	} else if (what.id=='divFavicon') {
		showPrev(images+'/Favicon.png','The page favicon will show the player state of playing, paused, stopped, or buffering for YouTube Flash videos.');
		...
		showPrev(images+'/RSS.png','Create a link to use with Google Reader for YouTube, Twitter, and Instagram.');
...
function loadConfig () {
	...
	var bits=['AutoPlayEmbed', 'ChangeResView', 'Cinemize', 'Cleanup', 'Comments', 'Description', 'DownloadLinks', 'ExpDescription', 'Favicon', 'Frame', 'Footer', 'Guide', 'Header', 'Headline', 'Hide', 'Hwebm', 'Hmp4', 'Hflv', 'H3gp', 'H3d', 'IgnoreTime', 'Loop', 'MoPlay', 'Playlist', 'Popout', 'Popup', 'RSS', 'SSL', 'ScrollTop', 'Sidebar', 'SidebarStay', 'Sites', 'Keys', 'Volume', 'Wads', 'Wmode', 'daily', 'dump', 'escapist', 'fear', 'funny', 'g4tv', 'hulu', 'meta', 'others', 'twitter', 'vimeo', 'youtube', 'eyoutube', 'youtubeHome', 'youtubeUser', 'youtubeChannel'];
...
function showhide (what) {
	...
	// check for new opera version
	if (thisbrowser=='opera' && what.id=='msupport') {
		checkOperaUpdate();
	}
...
function getVersion (thisbrowser) {
	...
	if (d.getElementById('version')&&vers!='') {
		d.getElementById('version').textContent='Version '+vers;
		setstor.Version=vers;

		// safari needs to push this save
		if (thisbrowser=='safari') { safari.self.tab.dispatchMessage('savePrefs',setstor); }
	}
}

function checkOperaUpdate () {
	// check for opera update

	var d=document;
	var vers = d.getElementById('version');
	if (vers && vers.textContent!='') {
		vers = vers.textContent.replace('Version ','');

		vers2 = getFile (thisbrowser,'https://spoi.com/software/yto/updates.oml');
		if (vers2.indexOf('updatecheck=')!='-1') {
			vers2 = vers2.split('updatecheck=')[1];
			if (vers2 && vers2!=vers) {
				d.getElementById('newversion').style.display='block';
				d.getElementById('newversion').innerHTML = 'You can download version <a href="https://spoi.com/software/yto/YouTube_Options_'+vers2+'.oex">'+vers2+'</a> now!';
			} else {
				d.getElementById('newversion').textContent='This is the most recent version.';
				d.getElementById('newversion').style.display='block';
			}
		} else {
			d.getElementById('newversion').style.display='block';
...
function addEvents (d) {
	...
	// mouse-over
	var maps = ['divAutoPlayDefault', 'divAutoPlayNo', 'divAutoPlayPlay', 'divAutoPlayPlaylist', 'divAutoPlayNoBuff', 'divAutoPlayNoBuffList', 'divAutoPlayEmbed', 'divChangeRes', 'divChangeView', 'divCinemize', 'divCleanup', 'divComments', 'divControlsTheme', 'divDescription', 'divDownloadLinks', 'divExpDescription', 'divFavicon', 'divFrame', 'divFooter', 'divGuide', 'divHeader', 'divHeadline', 'divHide', 'divIgnoreTime', 'divInVideoDefault', 'divInVideoAds', 'divInVideoShowcc', 'divInVideoAll', 'divLoop', 'divMoodDefault', 'divMoodMatte', 'divMoodDim', 'divMoPlay', 'divNormal', 'divPlaylist', 'divPopout', 'divPopup', 'divRSS', 'divScrollTop', 'divSidebar', 'divSidebarStay', 'divSSL', 'divSwatch', 'divSyncRes', 'divVolume', 'divWads', 'divKeys', 'divWmode', 'preDiv', 'showpmenu3', 'spoi', 'vf', 'vg', 'dltypes', 'advanced', 'secLinks', 'divLinksIn', 'divThemeDefault', 'divThemeLight', 'divThemeDark'];
