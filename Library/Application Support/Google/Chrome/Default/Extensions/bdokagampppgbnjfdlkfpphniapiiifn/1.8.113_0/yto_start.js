...
function ytFindObj (e) {
	...
	if (player && (eltDomain.indexOf('ytimg.com')!='-1' || eltDomain.indexOf('youtube.com')!='-1') && /^(embed|object)$/.test(player.localName)) {
	...
	} else if (player.nodeName!='SCRIPT' && player.baseURI.indexOf('youtube.com')!='-1' && window!=window.parent && ytOoptions && typeof ytOoptions=='object' && player.ownerDocument.head && player.ownerDocument.head.innerHTML.indexOf('ytOhtml5style')=='-1') {
		// embedded youtube on third party pages
		...
		if (ytOoptions.ControlsTheme=='light') {
			...
			temp.innerText='var themeCount=0; var d=document; var themeWait = setInterval(function(){var aa=d.getElementById(\'movie_player-html5\'); var ab=d.getElementById(\'video-player\'); themeCount++; if (aa || ab) {clearInterval(themeWait); if (aa) {var ac=aa.getElementsByClassName(\'html5-video-controls\');} if (ab) {var ac=ab.getElementsByClassName(\'html5-video-controls\');} if (ac && ac[0]) {ac[0].className+=\' light-theme\';}} if (themeCount>18){clearInterval(themeWait);}},1000);';
		...
		if (ytOoptions.Hide=='true') {
			...
			temp.setAttribute('type','text/javascript');
			temp.innerText='var hideCount=0; var d=document; var hideWait = setInterval(function(){var aa=d.getElementById(\'movie_player-html5\'); var ab=d.getElementById(\'video-player\'); if (aa || ab) {clearInterval(hideCount); if (aa) {aa.className+=\' autohide-embeds autohide-aspect hide-controls\';} if (ab) {ab.className+=\' autohide-embeds autohide-aspect hide-controls\';}} hideCount++; if (hideCount>18) {clearInterval(hideWait);}},1000);';
			player.ownerDocument.body.appendChild(temp);
		}
	}
}
