...
function changeSize (mode) {
	...
	if (vidtype=='youtube') {
		if (ytType=='watch7') {
			...
			newcss += '#watch7-container.watch-playlist #player {width: '+width+'px;} ';

			// correct and center css of auto-sizing larger videos
			newcss += '#watch7-container.watch-medium #player {width: '+width+'px;} ';
			newcss += '#watch7-container.watch-large #player {width: '+width+'px;} ';
			...
			newcss += '#player {width: auto;} ';

			// adjust height of playlist
			var plistheight = height;
			if (options.Hide=='false') {
				if (options.SidebarStay=='true') {plistheight = height + 30;}
			}
			...
			if (vidKind()=='html5') {
				// html5 video resize
				newcss += '.html5-video-content, .html5-video-player .video-stream {width:'+width+'px !important; height:'+height+'px !important;} ';
			}

			// add space for player controls
			var ytVidKind = vidKind();
			var cAreHidden = ytControlsHidden();
			if ((ytVidKind=='flash' && cAreHidden==false) || (ytVidKind=='html5' && options.Hide=='false')) {
				height+=30;
			} else if (ytVidKind=='html5' && options.Hide=='true') {
				height+=3;
			}

			// flash video resize
			newcss += '#player-api {width: '+width+'px !important; height: '+height+'px !important;} ';

			// make video center
			newcss += '#player-api {margin: 0 auto;} ';
			newcss += '#player {margin: 0 auto !important; padding-left: 0 !important;} ';
			...
			if (vidKind()=='html5') {
				...
				// make sure video shows in portal
				newcss += 'video {top: 0 !important;} ';
			}
		...
		if (ytType=='feather') {
			...
			// add space for player controls
			if ((ytControlsHidden()==true && options.Hide=='false')||vidKind()=='html5') {
				height+=30;
			}
...
function ytControlsHidden () {
	// determine if controlls are being shown (youtube)
	...
	if (ytVidType=='flash') {
		...
		if (el && el.getAttribute('flashvars')) {
			var aa = el.getAttribute('flashvars');
			if (aa.indexOf('autohide=1')!='-1' || aa.indexOf('autohide=3')!='-1') {
				// controls probably hidden
				return true;
			}
		}
	} else {
		// html5
		var aa=d.getElementById('movie_player-html5');
		if (aa && aa.className.indexOf('autohide-embeds autohide-aspect hide-controls')!='-1') {return true;}

		var aa=d.getElementById('movie_player');
		if (aa && aa.className.indexOf('autohide-embeds autohide-aspect hide-controls')!='-1') {return true;}
	}

	// controls probably showing
	return false;
}
...
function makeLinks (movieDiv,ytVidType) {
	...
	if (arr && arr.length!=0) {
		...
		if (linksadded>0) {
			// append download div to movie
			if (ytType=='watch7') {
				var parent = d.getElementById('player-api').parentNode;
				parent.insertBefore(links, d.getElementById('player-api').nextSibling);
			}
			if (ytType=='feather') {
...
function do_AutoPlay (player,ytVidType) {
	...
	if (ytVidType=='flash') {
		if (options.AutoPlay=='no' || (options.AutoPlay=='playlist' && inplaylist==false)) { player.pauseVideo(); setFavico('pause'); }
		...
		if (options.AutoPlay=='nobuff' || (options.AutoPlay=='nobufflist' && inplaylist==false)) {
			// no api for stopping html5
		}
	}
...
#watch-channel-discoverbox {-webkit-box-shadow: inset 0 4px 8px rgba(0,0,0,.05),0 1px 0 transparent; box-shadow: inset 0 4px 8px rgba(0,0,0,.05),0 1px 0 transparent;} ';
	...
	if (ytType=='watch7') {
		...
		newcss += '#watch7-player {margin: 0 auto;} #player.has-ad {left:0 !important;} ';
		...
		newcss += '#watch7-headline, #watch7-user-header, #watch7-content #watch-privacy-contain, #yt-masthead-container, #watch7-action-panels, #watch-discussion, #watch7-action-buttons {border-color: transparent !important;} ';
...
function do_baseCSS () {
	...
	if (vidtype=='youtube') {
		...
		if (ytType=='watch7') {newcss += '#footer {margin: auto;} .sidebar-expanded #watch7-main {width: 945px !important;} ';}
		...
		// hide iframes except for the necessary ones
		newcss += 'iframe {display:none;} .sb-card-content iframe {display: inline;} .picker iframe {display: inline;} ';

		// center title for non-clean videos
		newcss += '#watch-headline-title {text-align: center;} ';
		...
		if (options.ControlsTheme=='light') {
			...
			newcss += '.watch7-playlist-bar-left, .watch7-playlist-bar-secondary-controls, #watch7-playlist-bar-controls, .watch7-playlist-bar-notifications {background: #ccc;} ';
...
#comments-view .content {margin-left: 0;} #comments-view .comment.child {margin-left: 30px;} \
';
	...
	if (options.youtubeHome=='true' || options.youtubeChannel=='true') {
		...
		newcss += '.branded-page-v2-primary-col {min-height: auto;} ';
		// hide recommended
		newcss += '.ytOrec {display: none;} ';
	}
...
//	toggleOff('guide');
	...
	if (ytType=='watch7') {
		...
		newcss += '#player {background-image: none !important; background-color: transparent !important; background: transparent !important;} ';
		newcss += '#watch7-headline {border-color: transparent; background-color: transparent;} ';
		newcss += '#watch-discussion {height: 365px; overflow-y: scroll; overflow-x: hidden; border-right-color: transparent;} ';
		...
		// center video
		newcss += '#watch7-player {margin: 0 auto;} ';
		newcss += '.site-left-aligned.guide-enabled #player, .site-right-aligned.guide-enabled #player {margin: 0 auto; padding: 0; width: 100%;} ';
		newcss += '.watch-medium #player-api, .watch-large #player-api, #player-api {margin: 0 auto;} ';

		// hide featured suggested videos
		newcss += '.video-list-item .related-video-featured {display: none !important;} ';
		...
		newcss += '.watch-playlist #player {padding-top: 0;} ';
		...
		newcss += '#watch-discussion a {color: #999 !important;} #watch7-content:hover a, #watch7-sidebar:hover a, #watch7-sidebar:hover .title {color: #1C62B9 !important;} ';
		...
		newcss += '#watch7-container.watch-playlist #player {margin: 0 auto; padding: 0;} ';
		...
		newcss += '#watch7-container.watch-playlist #player {width: '+small+'px;} ';
		...
		newcss += '#watch7-container.watch-medium #player {width: '+med+'px;} ';
		newcss += '#watch7-container.watch-medium #watch7-playlist-container {width: '+med+'px;} ';
		newcss += '#watch7-container.watch-large #player {width: '+large+'px;} ';
		...
		// move up the video
		newcss += '#player, .watch7-playlist {padding-top: 0;} ';
...
//		newcss += '#watch7-video {width: 100% !important;} ';
		...
		if (options.Popout=='true') {
			...
			newcss += '.sidebar-expanded #watch7-headline {margin-right: -312px;} ';
		} else {
			newcss += '.sidebar-collapsed #watch7-headline {margin-right: -183px;} ';
			newcss += '.sidebar-expanded #watch7-headline {margin-right: -305px;} ';
		}
		...
		if (aa) {
			...
			d.getElementById('ytOdexp').addEventListener('click',function(){
				...
				var af = d.getElementById('watch-discussion');
...
function setFavico (kind) {
	...
	if (options.Favicon=='false') {return false;}
	...
	if (thisbrowser=='chrome') {
		...
		var icopause = 'data:image/vnd.microsoft.icon;base64,';
		var icoplay = 'data:image/vnd.microsoft.icon;base64,';
		var iconobuff = 'data:image/vnd.microsoft.icon;base64,';
		var icobuffering = 'data:image/vnd.microsoft.icon;base64,';

		var temp = d.createElement('link');
		temp.setAttribute('rel','shortcut icon');
		temp.setAttribute('type','image/x-icon');
		temp.setAttribute('id','ytOfavico');

		if (kind=='pause') { temp.href=icopause; }
		if (kind=='play') { temp.href=icoplay; }
		if (kind=='nobuff') { temp.href=iconobuff; }
		if (kind=='buffering') { temp.href=icobuffering; }
		d.head.appendChild(temp);
	}
}
...
function modSite (site) {
	...
	if (site=='youtube'){
		...
		if (options.youtubeHome=='true' && ytPage=='home') {
			...
			if (bits[0]) {
				for (var i=0;i<bits.length;i++) {
					if (bits[i].className.indexOf('feed-author-bubble')!='-1') {
						var thep = bits[i].parentNode.parentNode.parentNode;
						if (thep && thep.nodeName=='LI') {
		...
		if (options.RSS=='true') {
			...
			if (who != '' && d.getElementById('doc')) {
				...
				if (where[0]) {
					...
					addCSS('#ytOrss {padding-top: 10px; float: left; color: #0084B4;} '); // official twitter blue: #00bef6
				}
			}
		}
...
function makeFrame () {
	...
	if (options.Cleanup=='true') {
		...
		newcss += '#player {background: url('+newback+');} ';
	}
...
function addKeyControls () {
	...
	var d=getDOM();
	if (d.body) {d.body.addEventListener('keydown',function(event) {
...
function addFavicoCheck () {
	// add event listener for changing favico (youtube flash)

	var d=getDOM();
	var html5time = d.getElementsByClassName('html5-current-time');

	if (html5time && html5time[0]) {
		// html5

		/*
		var prevtime='0:00';
		var waiter5a = setInterval(function() {
			var thistime=html5time[0].textContent;
		...
		*/
		...
		var ytVidType = vidKind();
		var icoplayer = getPlayer(ytVidType);

		if (icoplayer) {
			var previco=0;
			var failico=0;
			var waiter5b = setInterval(function(){
				try {
					var mpstate = icoplayer.getPlayerState();
					if (mpstate != previco) {
						previco = mpstate;
					...
					// try and re-get the player
					ytVidType = vidKind();
					icoplayer = getPlayer(ytVidType);
				}
				if (failico>18) {console.log('YTO: there was a problem setting the favicon.'); clearInterval(waiter5b);}
			},1089);
		}
	}
}
